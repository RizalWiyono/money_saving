...
<body>
    <div class="main-container">
        <header>
            ...
            <ul>
                <li><a href="../home_user/">Saldo</a></li>
        ...
        <main>
            ...
            <div class="main-content">
                ...
                $query_saldo  = mysqli_query($connect, "SELECT SUM(nominal_saldo) as Max_Saldo , DATE_FORMAT(date, '%m') as Month, DATE_FORMAT(date, '%M') as Month_Name FROM `tb_add_saldo` WHERE id_biodata=$param GROUP BY DATE_FORMAT(date, '%m')");
                while($row = mysqli_fetch_array($query_saldo)){ 
                    $param_month = $row['Month'];
                    $query_withdraw  = mysqli_query($connect, "SELECT SUM(nominal_saldo) as Min_Saldo FROM `tb_withdraw` WHERE month(date) = '$param_month' && id_biodata=$param");
